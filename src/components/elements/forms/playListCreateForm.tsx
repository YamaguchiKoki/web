...
export function PlayListCreateForm({ close }: Props) {
  ...
  const schema = z.object({
    ...
    songs: z.array(
      z.object({ name: z.string(), url: z.string().url().nullable(), type: z.nativeEnum(urlType)})
    )
  });
            ...
            return (
              <div key={field.id} className="flex flex-row space-x-4 mt-5">
                ...
                <Controller 
                  ...
                  control={control}
                  render={({ field }) => (
                    <Input {...field} placeholder="name" name={`songs[${index + 1}][name]` }/>
                  )}
                />
                {/* url */}
                <Controller
                  ...
                  control={control}
                  render={({ field }) => (
                    <Input {...field} value={field.value || ''} placeholder="url" name={`songs[${index + 1}][url]`} />
                  )}
                />
                ...
                {/* delete button */}
                <Button onClick={() => remove(index)} variant="outline" disabled={fields.length === 1}>
                  delete
                </Button>
                {/* 一つ下に移動 */}
